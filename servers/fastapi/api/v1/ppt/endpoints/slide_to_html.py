...
from utils.template_llm_provider import should_use_local_model_for_templates, should_use_api_model_for_templates
...
async def generate_html_from_slide_with_llm_client(
    base64_image: str,
    media_type: str,
    xml_content: str,
    fonts: Optional[List[str]] = None,
    ...
    """
    Generate HTML content from slide image and XML using the configured LLM client.
    This function uses the same LLM provider as content generation.
    ...
    """
    print("Generating HTML from slide image and XML using configured LLM client...")
    try:
        llm_client = LLMClient()
        
        # Prepare the user message with image and text content
        fonts_text = (
        ...
        messages = [
            LLMMessage(role="system", content=GENERATE_HTML_SYSTEM_PROMPT),
            LLMMessage(role="user", content=user_text, image=base64_image)
        ]
        
        # Get the model to use - this will be the exact model you have installed/configured
        from utils.llm_provider import get_model
        model = get_model()
        
        print(f"Making LLM request for HTML generation using model: {model}")
        ...
        error_msg = str(e)
        print(f"Exception occurred during HTML generation: {error_msg}")
        print(f"Exception type: {type(e)}")
        if "timeout" in error_msg.lower():
            raise HTTPException(
                status_code=408,
                detail=f"LLM timeout during HTML generation: {error_msg}",
            )
        elif "connection" in error_msg.lower():
            raise HTTPException(
                status_code=503,
                detail=f"LLM connection error during HTML generation: {error_msg}",
            )
        else:
            raise HTTPException(
                status_code=500,
                detail=f"LLM error during HTML generation: {error_msg}",
            )


async def generate_react_component_from_html_with_llm_client(
    html_content: str,
    image_base64: Optional[str] = None,
) -> str:
    """
    Convert HTML content to TSX React component using the configured LLM client.
    This function uses the same LLM provider as content generation.
    ...
    try:
        ...
        # Create messages for the LLM
        user_content = f"HTML INPUT:\n{html_content}"
        messages = [
            LLMMessage(role="system", content=HTML_TO_REACT_SYSTEM_PROMPT),
            LLMMessage(role="user", content=user_content, image=image_base64)
        ]
        ...
        react_content = await llm_client.generate(
            model=model,
            messages=messages,
            max_tokens=4000
        )
        ...
        error_msg = str(e)
        print(f"Exception occurred during React generation: {error_msg}")
        print(f"Exception type: {type(e)}")
        if "timeout" in error_msg.lower():
            raise HTTPException(
                status_code=408,
                detail=f"LLM timeout during React generation: {error_msg}",
            )
        elif "connection" in error_msg.lower():
            raise HTTPException(
                status_code=503,
                detail=f"LLM connection error during React generation: {error_msg}",
            )
        else:
            raise HTTPException(
                status_code=500,
                detail=f"LLM error during React generation: {error_msg}",
            )
...
async def convert_slide_to_html(request: SlideToHtmlRequest):
    ...
    try:
        ...
        # Generate HTML using the appropriate function based on configuration
        if should_use_local_model_for_templates():
            print("Using local model for template generation")
            html_content = await generate_html_from_slide_with_llm_client(
            ...
            print("Using API model (OpenAI GPT-5) for template generation")
...
async def convert_html_to_react(request: HtmlToReactRequest):
    ...
    try:
        ...
        # Convert HTML to React component using the appropriate function based on configuration
        if should_use_local_model_for_templates():
            print("Using local model for React component generation")
            react_component = await generate_react_component_from_html_with_llm_client(
                html_content=request.html,
                image_base64=image_b64,
            )
        else:
            print("Using API model (OpenAI GPT-5) for React component generation")
