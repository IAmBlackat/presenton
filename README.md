...
The project includes both `docker-compose.yml` (legacy format) and `docker-compose.yaml` (modern format) files. Use `docker-compose.yaml` for new deployments. GPU services have been removed for CPU-only deployments.
